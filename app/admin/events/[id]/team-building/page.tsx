...
export default function TeamBuildingPage() {
  ...
  const getSessionStatus = (session: Session) => {
    ...
    if (session.groups.length === 0) {
      return { status: 'empty', label: 'Aucune équipe', color: 'text-gray-500', icon: AlertCircle }
    }
    if (unassigned > 0) {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold mb-2">🤝 Gestion des équipes Team Building</h1>
        <p className="text-muted-foreground">
          Créez et gérez les équipes pour vos activités de Team Building
        </p>
      </div>
      ...
      {filteredSessions.length === 0 ? (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">Aucune session Team Building</h3>
            <p className="text-sm text-muted-foreground mb-4">
              Créez une session de type Team Building avec l&apos;option &quot;Session avec groupes&quot; activée depuis le module Programme
            </p>
              ...
              <Card key={session.id} className="hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {session.groups.length} équipe{session.groups.length > 1 ? 's' : ''}
                        </span>
                        <span className="text-muted-foreground">•</span>
                        <span className="text-sm text-muted-foreground">
                    ...
                    {/* Right: Action Button */}
                    <Button
                      onClick={() => router.push(`/admin/events/${eventId}/team-building/${session.id}`)}
                      className="ml-4"
                    >
                      Gérer
